...
// run time.
//
// `openabstractions serve <capability>` replaces asksd, rightsd, routerd and
// `jobd run` as the thing a platform starts. It is not one host process — every
// platform's stated reason runs against sharing one, and the memory a shared
// host saves was inside the noise (research/svc195/PROPOSAL.md §§ 1, 5).
// Registering it is per platform and is not here: the registration decides when
// a capability starts and when it idles out, and this program only has to be
// something a registration can name.
//
// The CLIs are untouched. dl, jobctl, asks, rights, router and keepawake are
// what an adopter types, and none of them moved.
//
// # Where this file will live
//
// Not here. It belongs to no single layer, so by our own naming rule it belongs
// in a repository of its own — the shape service-jobd already has. It sits in
// router/go because that is the only tree this task owns that no repository
// publishes, so putting it here breaks no published module's build: a package
// under download/go, asks/go or rights/go would make that layer's published
// go.mod require the other three, and service-router does not exist as a
// repository for any of them to require. Moving it is a manifest line, not a
// code change.
package main
