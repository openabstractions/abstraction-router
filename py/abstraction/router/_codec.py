...
PROFILES = ["chat", "embed", "transcription", "speech", "image"]


VERDICTS = ["resident", "would-load", "hosted", "unservable", "not-here", "unparseable", "unauthorised", "no-host"]


WIRE_KINDS = ["openai-compatible", "anthropic-messages", "deepgram-prerecorded", "elevenlabs-stream", "stability-v2beta", "fal-queue", "replicate-predictions", "openai-realtime", "oa-remote@1"]
...
class PickRequest:
    ...
    model: str = ""
    fresh: bool = False
    allowed: HostAllowance | None = None
    profile: str = ""
...
@dataclasses.dataclass(kw_only=True)
class Observation:
    caller: Caller = dataclasses.field(default_factory=Caller)
    took_ms: int = 0
    cache_age_ms: int = 0
...
class ModelsSnapshot:
    observation: Observation = dataclasses.field(default_factory=Observation)
    models: list[Family] = dataclasses.field(default_factory=list)
...
class HostState:
    ...
    why: str = ""
    installed: int = 0
    resident: list[str] = dataclasses.field(default_factory=list)
    servable: bool = False
    hosted: bool = False
    wire: str = ""
    credential: str = ""
...
class HostsSnapshot:
    observation: Observation = dataclasses.field(default_factory=Observation)
    hosts: list[HostState] = dataclasses.field(default_factory=list)
    doubled: list[str] = dataclasses.field(default_factory=list)
    asked: list[Ask] = dataclasses.field(default_factory=list)
...
class Decision:
    ...
    model: str = ""
    endpoint: str = ""
    installed_on: list[str] = dataclasses.field(default_factory=list)
    authorised: HostAllowance | None = None
    withheld: list[str] = dataclasses.field(default_factory=list)
    loads: int = 0
    why: str = ""
...
class PickResult:
    observation: Observation = dataclasses.field(default_factory=Observation)
    decision: Decision = dataclasses.field(default_factory=Decision)


@dataclasses.dataclass(kw_only=True)
class _RouterModelsArguments:
    fresh: bool = False


@dataclasses.dataclass(kw_only=True)
class _RouterHostsArguments:
    fresh: bool = False


@dataclasses.dataclass(kw_only=True)
class _RouterPickArguments:
    request: PickRequest = dataclasses.field(default_factory=PickRequest)
...
@dataclasses.dataclass(kw_only=True)
class _RouterModelsResult:
    value: ModelsSnapshot = dataclasses.field(default_factory=ModelsSnapshot)


@dataclasses.dataclass(kw_only=True)
class _RouterHostsResult:
    value: HostsSnapshot = dataclasses.field(default_factory=HostsSnapshot)


@dataclasses.dataclass(kw_only=True)
class _RouterPickResult:
    value: PickResult = dataclasses.field(default_factory=PickResult)


def _write_host_allowance(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "hosts")
    out += b": "
    _strs(out, v.hosts, depth + 1)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_pick_request(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "model")
    ...
    if v.allowed is not None:
        ...
        _esc(out, "allowed")
        out += b": "
        _write_host_allowance(out, v.allowed, depth + 1)
    if v.profile:
        out += b","
        out += b"\n"
        _pad(out, depth + 1)
...
def _write_caller(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "user_description")
    out += b": "
    _esc(out, v.user_description)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "path_description")
    out += b": "
    _esc(out, v.path_description)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_observation(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "caller")
    out += b": "
    _write_caller(out, v.caller, depth + 1)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "took_ms")
    ...
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "cache_age_ms")
    out += b": "
    _num(out, v.cache_age_ms)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_alias(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "host")
    ...
    if v.hosted:
        ...
        _esc(out, "hosted")
        out += b": "
        out += b"true" if v.hosted else b"false"
    if v.profiles:
        out += b","
        out += b"\n"
        _pad(out, depth + 1)
...
def _write_models_snapshot(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "observation")
    out += b": "
    _write_observation(out, v.observation, depth + 1)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "models")
    out += b": "
    _write_list(out, v.models, depth + 1, _write_family)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_host_state(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "host")
    ...
    _esc(out, "installed")
    out += b": "
    _num(out, v.installed)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "resident")
    out += b": "
    _strs(out, v.resident, depth + 1)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "servable")
    ...
    if v.declared_by:
        ...
        out += b"\n"
        _pad(out, depth + 1)
        _esc(out, "declared_by")
        out += b": "
        _esc(out, v.declared_by)
    if v.profiles:
        out += b","
        out += b"\n"
        _pad(out, depth + 1)
...
def _write_ask(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "at")
    out += b": "
    _esc(out, _write_timestamp(v.at))
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "caller")
...
def _write_hosts_snapshot(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "observation")
    ...
    _esc(out, "hosts")
    out += b": "
    _write_list(out, v.hosts, depth + 1, _write_host_state)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "doubled")
    out += b": "
    _strs(out, v.doubled, depth + 1)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "asked")
    out += b": "
    _write_list(out, v.asked, depth + 1, _write_ask)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_decision(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "asked")
    ...
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "installed_on")
    out += b": "
    _strs(out, v.installed_on, depth + 1)
    if v.authorised is not None:
        ...
        _esc(out, "authorised")
        out += b": "
        _write_host_allowance(out, v.authorised, depth + 1)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "withheld")
    out += b": "
    _strs(out, v.withheld, depth + 1)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "loads")
...
def _write_pick_result(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "observation")
    ...
    _esc(out, "decision")
    out += b": "
    _write_decision(out, v.decision, depth + 1)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_oa_router_models_arguments(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "fresh")
...
def _write_oa_router_hosts_arguments(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "fresh")
...
def _write_oa_router_pick_arguments(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "request")
    out += b": "
    _write_pick_request(out, v.request, depth + 1)
    out += b"\n"
    _pad(out, depth)
    out += b"}"
...
def _write_oa_router_models_result(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "value")
    out += b": "
    _write_models_snapshot(out, v.value, depth + 1)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_oa_router_hosts_result(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "value")
    out += b": "
    _write_hosts_snapshot(out, v.value, depth + 1)
    out += b"\n"
    _pad(out, depth)
    out += b"}"


def _write_oa_router_pick_result(out, v, depth):
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "value")
    out += b": "
    _write_pick_result(out, v.value, depth + 1)
    out += b"\n"
    _pad(out, depth)
    out += b"}"
        ...
        while True:
            ...
            if key == "model":
                ...
                if seen & 4:
                    raise r.refuse("duplicate_field")
                seen |= 4
                v.allowed = _read_host_allowance(r)
            elif key == "profile":
                if seen & 8:
                    raise r.refuse("duplicate_field")
                seen |= 8
        ...
        while True:
            ...
            if key == "observation":
                if seen & 1:
                    raise r.refuse("duplicate_field")
                seen |= 1
                v.observation = _read_observation(r)
            elif key == "models":
                if seen & 2:
                    raise r.refuse("duplicate_field")
                seen |= 2
                v.models = _read_list(r, _read_family)
            else:
                r.skip_value()
            r.ws()
            if r.at() != _COMMA:
...
def _read_host_state(r):
    ...
    if r.at() != _RBRACE:
        while True:
            ...
            elif key == "declared_by":
                if seen & 1024:
                    raise r.refuse("duplicate_field")
                seen |= 1024
                v.declared_by = r.string()
            elif key == "profiles":
                if seen & 2048:
                    raise r.refuse("duplicate_field")
                seen |= 2048
        ...
        while True:
            ...
            if key == "observation":
                ...
                if seen & 2:
                    raise r.refuse("duplicate_field")
                seen |= 2
                v.hosts = _read_list(r, _read_host_state)
            elif key == "doubled":
                if seen & 4:
                    raise r.refuse("duplicate_field")
                seen |= 4
                ...
                if seen & 8:
                    raise r.refuse("duplicate_field")
                seen |= 8
                v.asked = _read_list(r, _read_ask)
            else:
                r.skip_value()
            r.ws()
            if r.at() != _COMMA:
        ...
        while True:
            ...
            if key == "asked":
                ...
                if seen & 128:
                    raise r.refuse("duplicate_field")
                seen |= 128
                v.authorised = _read_host_allowance(r)
            elif key == "withheld":
                if seen & 256:
                    raise r.refuse("duplicate_field")
                seen |= 256
...
def _read_oa_router_models_arguments(r):
    if r.at() != _LBRACE:
        raise r.refuse("wrong_type")
    r.enter()
    r.pos += 1
    v = _RouterModelsArguments()
    seen = 0
    r.ws()
    if r.at() != _RBRACE:
        while True:
...
def _read_oa_router_hosts_arguments(r):
    if r.at() != _LBRACE:
        raise r.refuse("wrong_type")
    r.enter()
    r.pos += 1
    v = _RouterHostsArguments()
    seen = 0
    r.ws()
    if r.at() != _RBRACE:
        while True:
...
def _read_oa_router_pick_arguments(r):
    if r.at() != _LBRACE:
        raise r.refuse("wrong_type")
    r.enter()
    r.pos += 1
    v = _RouterPickArguments()
    seen = 0
    r.ws()
    if r.at() != _RBRACE:
        while True:
            ...
            if key == "request":
                if seen & 1:
                    raise r.refuse("duplicate_field")
                seen |= 1
                v.request = _read_pick_request(r)
            else:
                raise r.refuse("unknown_field")
            r.ws()
            if r.at() != _COMMA:
...
def _read_oa_router_models_result(r):
    if r.at() != _LBRACE:
        raise r.refuse("wrong_type")
    r.enter()
    r.pos += 1
    v = _RouterModelsResult()
    seen = 0
    r.ws()
    if r.at() != _RBRACE:
        while True:
            ...
            if key == "value":
                if seen & 1:
                    raise r.refuse("duplicate_field")
                seen |= 1
                v.value = _read_models_snapshot(r)
            else:
                raise r.refuse("unknown_field")
            r.ws()
            if r.at() != _COMMA:
...
def _read_oa_router_hosts_result(r):
    if r.at() != _LBRACE:
        raise r.refuse("wrong_type")
    r.enter()
    r.pos += 1
    v = _RouterHostsResult()
    seen = 0
    r.ws()
    if r.at() != _RBRACE:
        while True:
            ...
            if key == "value":
                if seen & 1:
                    raise r.refuse("duplicate_field")
                seen |= 1
                v.value = _read_hosts_snapshot(r)
            else:
                raise r.refuse("unknown_field")
            r.ws()
            if r.at() != _COMMA:
...
def _read_oa_router_pick_result(r):
    if r.at() != _LBRACE:
        raise r.refuse("wrong_type")
    r.enter()
    r.pos += 1
...
_REFUSALS = ["malformed", "bad_string", "number_spelling", "wrong_type", "bad_timestamp", "depth_exceeded", "duplicate_key", "duplicate_field", "unknown_field", "missing_field", "trailing_bytes"]
...
_SERVICE_RECORDS = {
    ...
    "OARouterModelsArguments": (_RouterModelsArguments, [("fresh", "bool", "never"), ]),
    "OARouterHostsArguments": (_RouterHostsArguments, [("fresh", "bool", "never"), ]),
    "OARouterPickArguments": (_RouterPickArguments, [("request", "PickRequest", "never"), ]),
    ...
    "OARouterModelsResult": (_RouterModelsResult, [("value", "ModelsSnapshot", "never"), ]),
    "OARouterHostsResult": (_RouterHostsResult, [("value", "HostsSnapshot", "never"), ]),
    "OARouterPickResult": (_RouterPickResult, [("value", "PickResult", "never"), ]),
}
...
class RouterClient(Router):
    """Calls abstraction.router/router@1 through a transport that exchanges frames."""
    ...
    def models(self, fresh: bool) -> ModelsSnapshot:
        ...
        _args = _RouterModelsArguments()
        _args.fresh = fresh
        _arguments = _service_encode(_write_oa_router_models_arguments, _args, 1)
        _service_decode(_read_oa_router_models_arguments, _arguments, 1)
        _request = _service_request("abstraction.router/router@1", "Models", _arguments)
        _payload = _service_response(self._transport.exchange_frame(_request), "abstraction.router/router@1", "Models")
        _result = _service_decode(_read_oa_router_models_result, _payload, 1)
        return _result.value

    def hosts(self, fresh: bool) -> HostsSnapshot:
        _service_check("bool", fresh)
        _args = _RouterHostsArguments()
        _args.fresh = fresh
        _arguments = _service_encode(_write_oa_router_hosts_arguments, _args, 1)
        _service_decode(_read_oa_router_hosts_arguments, _arguments, 1)
        _request = _service_request("abstraction.router/router@1", "Hosts", _arguments)
        _payload = _service_response(self._transport.exchange_frame(_request), "abstraction.router/router@1", "Hosts")
        _result = _service_decode(_read_oa_router_hosts_result, _payload, 1)
        return _result.value

    def pick(self, request: PickRequest) -> PickResult:
        _service_check("PickRequest", request)
        _args = _RouterPickArguments()
        _args.request = request
        _arguments = _service_encode(_write_oa_router_pick_arguments, _args, 1)
        _service_decode(_read_oa_router_pick_arguments, _arguments, 1)
        _request = _service_request("abstraction.router/router@1", "Pick", _arguments)
        _payload = _service_response(self._transport.exchange_frame(_request), "abstraction.router/router@1", "Pick")
        _result = _service_decode(_read_oa_router_pick_result, _payload, 1)
        return _result.value
